...
app = Flask(__name__)
# CORS(app, resources={r"/*": {"origins": ["http://localhost:4200", "http://127.0.0.1:4200", "http://localhost:5000", "http://127.0.0.1:5000"], "methods": ["GET", "POST", "OPTIONS"]}}, allow_headers=["Access-Control-Allow-Methods", "Access-Control-Allow-Origin", "Content-Type", "Access-Control-Allow-Headers"], supports_credentials=True)
CORS(app, resources={r"/*": {
    "origins": ["http://localhost:4200", "http://127.0.0.1:4200"],
    "methods": ["GET", "POST", "OPTIONS"],
    "allow_headers": ["Content-Type", "Access-Control-Allow-Headers", "Access-Control-Allow-Origin", "Access-Control-Allow-Methods"],
...
def getConversation():
    if request.method == 'OPTIONS':
        return 'OPTIONS REQUEST', 200
    fetched_prompt = request.json.get('prompt', '')
    fetched_mode = request.json.get('mode', '')

    # threading.Thread(target=save_pdf_to_text).start()
    if fetched_prompt == 'pdf':
        fetched_prompt = open("pdf_to_text.txt").read()

    more_prompt = 'Make it a casual conversation.'
    ...
    if fetched_mode == 'yell':
        more_prompt = 'Make it in a yelling tone. The conversation should sound like a fight. Use CAPS LOCK for yelling.'
    ...
    gemini_resp_quiz = gemini_client.models.generate_content(
        model = "gemini-2.0-flash",
        contents = 'Generate a quiz (simple). Skip the extra dialogues. Jump straight to the questions. Dont format any text. Give the response in this format: [{"q1": "answer"}, {"q2": "answer"}, {"q3": "answer"}, ...]. Use this topic: ' + fetched_prompt
    )
